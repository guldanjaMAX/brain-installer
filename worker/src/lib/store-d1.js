...
export async function search(env, { query, embedding, limit = 10, filters = {}, weights = {}, rrfK = RRF_K, scope = null }) {
  ...
  //
  // For a SCOPED reader an empty vector list usually means their own scope
  // removed the candidates, and reporting that as degradation turns the flag
  // into a corpus probe: ask about a restricted subject, watch the flag flip,
  // and learn that the household holds a dense cluster of those documents
  // without ever seeing a byte of text. So that ONE branch is suppressed for a
  // scoped reader. A genuine outage is still reported, because projection
  // readiness is independent of who is asking.
  ...
  } else if (embedding && vec.length === 0 && kw.length > 0 && !scoped) {
    degraded = "vector";
