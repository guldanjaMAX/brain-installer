/**
 * recovery-page — the way back in for an owner who has lost every device.
 *
 * WHY THIS FILE EXISTS AT ALL
 *
 * This is the previous owner page, kept verbatim below the header, and it is
 * here because two lines of development met. One line added recovery codes: a
 * printed card, and the screen that accepts a code and mints a fresh passkey on
 * whatever device the owner is holding. The other line replaced the owner page
 * with a React app that has no such screen, having never had the feature to
 * carry. Taking the React app and nothing else would have deleted the only
 * route back into a brain whose devices are all gone, leaving the admin key as
 * the sole remedy — which is a key most owners do not hold.
 *
 * So /app is the React app, and this page is the escape hatch at /app/recover.
 * Not one byte of the ceremony was rewritten to put it here: the same reviewed
 * markup and the same inline script that shipped before the merge are what a
...
export function recoveryPageHtml(env) {
