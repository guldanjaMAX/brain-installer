...
try {
  ...
  const servicedCompiler = join(fakeWindowsRoot, "component-store-csc.exe");
  linkSync(fakeCompiler, servicedCompiler);
  assert.equal(lstatSync(fakeCompiler).nlink, 2, "a serviced SystemRoot compiler may be hard-linked");
  let compileRuns = 0;
  ...
  const linkedSourceRoot = join(fakeWindowsRoot, "linked-source");
  const linkedSource = join(linkedSourceRoot, "windows-dpapi.cs");
  mkdirSync(linkedSourceRoot);
  writeFileSync(linkedSource, "fixed source fixture", "utf8");
  linkSync(linkedSource, join(linkedSourceRoot, "source-copy.cs"));
  assert.throws(
    () => prepareWindowsDpapiSession({ ...sessionOptions, sourcePath: linkedSource }),
    /invalid DPAPI source file/,
  );

  const artifactOutside = join(fakeWindowsRoot, "linked-build-artifact-copy");
  const artifactSpawn = (command, args) => {
    if (String(command).endsWith("icacls.exe")) {
      return { status: 0, stdout: Buffer.alloc(0), stderr: Buffer.alloc(0) };
    }
    const output = String(args.find((value) => String(value).startsWith("/out:"))).slice(5);
    writeFileSync(output, "fixed compiled helper bytes", "utf8");
    const linkedArtifact = join(output, "..", "compiler-sidecar.tmp");
    writeFileSync(linkedArtifact, "fixed sidecar bytes", "utf8");
    linkSync(linkedArtifact, artifactOutside);
    return { status: 0, stdout: Buffer.alloc(0), stderr: Buffer.alloc(0) };
  };
  assert.throws(
    () => prepareWindowsDpapiSession({ ...sessionOptions, spawnSync: artifactSpawn }),
    /unexpected DPAPI build artifact/,
  );
