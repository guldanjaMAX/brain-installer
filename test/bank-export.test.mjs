...
//      to end.
//   5. THE WORKER BUNDLE. Git must present its source and committed output as LF
//      on every platform because the builder hashes and compares exact bytes.
//
...
const FIXTURES = join(ROOT, "test", "fixtures", "bank");
const bytes = (name) => readFileSync(join(FIXTURES, name));
const read = (name, options = {}) => readBankExport(bytes(name), { name, ...options });

/* ===================== cross-platform generated bytes ===================== */
{
  const buildFiles = ["ingest/bank-export.mjs", "worker/src/lib/bank-export.js"];
  const attributes = execFileSync(
    "git", ["check-attr", "text", "eol", "--", ...buildFiles],
    { cwd: ROOT, encoding: "utf8" },
  );
  for (const file of buildFiles) {
    check(`${file} is normalized as text on every Git checkout`,
      attributes.includes(`${file}: text: set`), attributes);
    check(`${file} is pinned to LF when Windows core.autocrlf is enabled`,
      attributes.includes(`${file}: eol: lf`), attributes);
    check(`${file} currently contains no CRLF bytes`,
      !readFileSync(join(ROOT, file)).includes(13), file);
  }
}
