...
/* ---- full acceptance independently enforces the deployed version ---- */
{
  ...
  check("the full acceptance suite rejects an old Worker version",
    suite.results[0]?.status === "fail" && /expected version 0\.1\.10/.test(suite.results[0]?.detail || ""),
  ...
  try {
    ...
    await cmdUpgrade(manifestPath, {
      ...
      cmdTest: async (path, options) => {
        executionPaths.add(path);
        events.push("test");
        check("upgrade acceptance requires the running package version", options.expectVersion === "0.1.10");
      },
      commitManifestVersion: (path, version) => {
        events.push("manifest");
        check("manifest advances only to the running package version", version === "0.1.10");
...
/* ---- manifest version commit is atomic and verified ---- */
{
  ...
  try {
    ...
    commitManifestVersion(manifestPath, "0.1.10");
    const committed = JSON.parse(readFileSync(manifestPath, "utf8"));
    check("the local manifest records the verified package version", committed.brain.version === "0.1.10");
...
{
  check("semantic versions compare numeric components", compareSemver("0.1.10", "0.1.9") > 0);
  ...
  try {
    ...
    await cmdUpgrade(legacyPath, {
      ...
      d1Query: async (_account, _database, sql) => {
        ...
        if (/UPDATE install_state/i.test(sql)) d1Version = "0.1.10";
        if (/SELECT product_version FROM install_state/i.test(sql)) {
          return { results: [{ product_version: d1Version }] };
        }
        return { results: [] };
    ...
    check(
      ...
      JSON.parse(readFileSync(legacyPath, "utf8")).brain.version === "0.1.10",
    );
  } finally {
    rmSync(sandbox, { recursive: true, force: true });
  }
