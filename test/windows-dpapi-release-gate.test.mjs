...
test("the Windows release gate uses the production probe for exactly 25 fresh rounds", () => {
  ...
  assert.match(gate, /writeAdminKeyFile\(destination, first, options\)/);
  assert.match(gate, /readAdminKeyFile\(destination, options\) !== first/);
  assert.match(gate, /writeAdminKeyFile\(destination, replacement, options\)/);
  assert.match(gate, /replacementStored\.includes/);
