...
const CATALOG = [
  ...
  entry("AUTH_REQUIRED", "A sign-in or credential is still needed", "This step reached a protected service without a usable authorization.", "The installer paused before relying on missing access.", "safe_after_step", ["Return to the matching technician or connection step for Cloudflare, Plaid, QuickBooks, Google, Zoom, or IMAP.", "Enter any sensitive value only in the provider page or hidden terminal prompt, then retry."], "It is unclear which account or provider step is missing."),
  ...
  entry("SAFETY_REVIEW_REQUIRED", "A safety check needs review", "The step reached a guard that needs review, or a one-time provider result could not be confirmed.", "The installer kept the current evidence available and did not assume that an uncertain provider action succeeded or failed.", "review_first", ["Keep this action paused. Check its current state before trying it again.", "Review the latest status or preview with the technician, then follow the exact recovery step for this issue."], "The current state is unclear, a preview is surprising, or the recovery step does not match what happened."),
