...
// Schema 17 added two provenance COLUMNS to `documents` (text_source,
// text_reliable) so an OCR'd document is distinguishable from one read from a
// text layer. No new table, and the vector protocol is unchanged, but the
// recovery contract tracks the EXACT current schema by design: a drill against
// a database one migration behind would export a column set that does not
// match the reviewed list, and refusing is the whole point of pinning it.
