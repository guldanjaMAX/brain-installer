...
| Plaid bank feed | **Built, read-only, and off by default.** `brain connect bank <manifest>` performs a manifest-only readiness check and opens the owner page. The owner completes Plaid Link, then assigns every masked account to one active owned entity. No Item-level or `primary` default is used. Transactions Sync stages a complete update window before ledger rows and the cursor become visible together; missing or retired assignment authority blocks both. Later cross-entity reassignment is refused once ledger history exists and needs a separate reviewed correction. History stays visibly partial through Plaid's NOT_READY and INITIAL states. Signed webhook receipt and reconciliation debt commit together, scheduled reconciliation remains the fallback, update mode needs a provider health check, and an unclear removal response is checked before another removal attempt. Credential-free fake and real SQLite/D1-compatible tests pass. No Plaid Sandbox, Production, real institution, or primary-bank proof exists yet |
...
| QuickBooks Online | **Sandbox-ready, not live-proven.** Fifteen accounting record types are read as company-fingerprint-bound snapshots. Intuit's broad Accounting consent is disclosed even though runtime calls are read-only. The client-owned HTTPS callback core is implemented and locally tested, but Production stays unavailable until CLI wiring and Cloudflare/Intuit field acceptance. Disconnect keeps imported documents, and QuickBooks queries do not expose reliable deletion truth |
...
**Sandbox-ready, not proven against an Intuit sandbox or production company.** The sandbox OAuth connection defaults to Intuit's documented `localhost` callback. The listener still binds only local IPv4 loopback. Intuit offers one broad Accounting consent that can authorize reads and writes; Financial Brain discloses that scope and performs query/read calls only. The protected credential, explicit source, environment, cursor, configuration receipt, reconciliation evidence, and document identities all use the same canonical company fingerprint. A different company cannot silently replace an existing source. Each run reads Accounts, Customers, Vendors, Invoices, Payments, Bills, Purchases, Journal Entries, Deposits, Transfers, Credit Memos, Bill Payments, Estimates, Sales Receipts, and Refund Receipts with stable company-separated IDs and provider timestamps. The query API does not provide reliable deletion truth, so each run is an explicitly partial present-record snapshot and never removes an old record merely because it was absent. Disconnect revokes access and keeps imported documents; removal needs a separately reviewed `brain forget` preview. The client-owned HTTPS callback and single-use local handoff core are implemented behind explicit enablement gates. Production still refuses before credential or browser access because CLI token-exchange/save wiring, the final hostname, Cloudflare request-log review, and Intuit field acceptance remain open.
