...
| Zoom | **Built.** `brain connect zoom`: a webhook on your own worker loads each cloud-recording transcript automatically, and a transcript that fails to load is retried and then reported rather than lost. **Needs a paid (Licensed) Zoom seat** — the free tier cannot cloud record at all. New recordings only, no backfill. Not yet run against a real Zoom account |
...
**What live capture actually needs, stated plainly:** a Mac or a Windows PC, and a daemon binary you build yourself. On a Mac the capture process is kept alive by a per-user LaunchAgent. On Windows it is kept alive by a Scheduled Task registered for your own account, and which shape of task your PC allows is checked at install time rather than assumed, because it depends on your machine's policy and not on your Windows version. Whatever is available gets installed and named:

- **Best case,** a task that runs whether you are signed in or not: capture keeps going after you sign out, and no console window ever appears.
- **Usual case for an ordinary account,** a task that starts when you sign in: it restarts capture on its own if it stops, but capture runs only while you are signed in, and each background process shows a console window you can minimise but should not close.
- **Locked-down machine where Windows will not register a task at all:** a Startup-folder launcher is installed instead. It starts capture when you sign in and it will **not** restart it. That is not supervision and nothing here calls it that: no refresh expectation is set, and `brain sources` reports WhatsApp as having no refresh scheduled rather than reporting it as live. The once-a-minute load cannot run this way either, so captured messages sit in the local file on your PC until you run `brain ingest <manifest> --from whatsapp` yourself.
- **Nothing available at all:** nothing is installed and `brain connect whatsapp` refuses, rather than leaving you believing capture is running.

None of the Windows behaviour above has run on a real Windows PC yet; it is tested against the generated task definition and the install and uninstall logic, not against Task Scheduler itself.

There is also no download-on-connect on either platform: how a signed binary should reach a client machine is an open decision, so today you either build it from this checkout (`cd daemons/whatsapp && ./build.sh`) or point the command at a binary you already have with `--daemon`. If it is absent, `brain connect whatsapp` says so and installs nothing.
...
**When a transcript does not load, you are told.** Zoom requires the webhook to be answered within seconds, so your worker answers first and fetches the transcript afterwards. Zoom treats an answered delivery as finished and never sends it again, which means every failure after that answer used to be invisible. It is not any more. The moment your worker answers Zoom, it writes down that the transcript is owed. If the fetch fails, the transcript stays owed, is retried automatically on the schedule your brain already runs, and if it is still missing an hour later the `zoom` source reads **BROKEN** in `brain sources` and `brain health`, fails the acceptance checklist, and adds a line to every answer your brain gives naming the meeting that is missing. You find out because you asked your brain something else, not because you happened to go looking. After about a day of failed retries it is written off and says why — most often that Audio Transcript is off in your Zoom recording settings, in which case no transcript was ever written and turning it on only helps future calls.

**What still cannot be seen, said plainly.** All of the above covers deliveries Zoom actually made to your worker. If a webhook never arrives at all — your worker was down long enough to exhaust Zoom's own retries, the Event Subscription was removed, or the call happened before you connected — nothing in your brain knows the meeting existed, and its absence looks exactly like a quiet week. Catching that would need your brain to poll your Zoom account's own list of recordings and compare, which needs a Zoom permission this connector does not ask for. It is not built and it is not claimed.
