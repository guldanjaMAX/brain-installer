...
`migrations/d1/0017_extraction_provenance.sql`, `test/ocr.test.mjs`,
...
So migration 0017 promotes two columns onto `documents`, following the
...
1. **A statement-to-rows extraction pass** (WP-14) that reads transactions from
   prose rather than from columns, and lands anything it cannot parse as
   `unparsed` and flagged rather than guessed. This is the expensive part, and
   it belongs with the `fin_*` schema, which is blocked on a decision that is
   not a unilateral build call.
...
3. **Two provenance vocabularies kept separate.** WP-11's `text_source` answers
   *how the text was obtained*. WP-14's `provenance` answers *where the figure
   came from*. Collapsing them would have to be unpicked; they are already
   distinct.
4. **A numeric confidence WP-14 can threshold** — provided, per document, with
