...
| Plaid hosted bank feed | Built | Scripted I/O and real SQLite/D1-compatible runtime | None, no Plaid credential or Sandbox call used | With an approved client-owned Plaid Sandbox account, run the disposable runner, then drive the Financial Brain routes and D1 to prove Link CSP, stable lost-response replay, one-winner exchange concurrency, per-Item NOT_READY to INITIAL to HISTORICAL partial-state progression, whole-window promotion, pending-to-posted replacement, provider-expiry-bound webhook verification, atomic replay-repairable reconciliation debt, health-proven update mode, unknown removal recovery, and provider-confirmed Item removal. |
...
| QuickBooks Online | Sandbox-ready company-bound read runner; production callback unavailable | Unit and scripted provider I/O for source custody, reconnect, crash-safe refresh fencing, cursor/disconnect races, expiry, revoke failure, and callback construction | None | Use an Intuit sandbox company to prove broad Accounting consent, returned company identity, same-company reconnect, deliberate wrong-company refusal, refresh rotation, all configured entities, pagination, changed records, outage retry, disconnect retention, retrieval, and a separately reviewed forget preview. Query snapshots cannot prove deletions. |
